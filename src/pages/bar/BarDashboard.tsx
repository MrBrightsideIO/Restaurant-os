// Bar dashboard for drink orders
...
export function BarDashboard({ onBack }: BarDashboardProps) {
    ...
    const loadData = async () => {
      try {
        const [ordersData, tablesData] = await Promise.all([
          OrderService.getKitchenOrders(), // Reuse kitchen orders service
          TableService.getAllTables()
        ]);
        
        // Filter orders that have drink items
        const drinkOrders = ordersData.filter(order => 
          order.items.some(item => item.menuItem.category === 'drink')
        );
        
        setOrders(drinkOrders.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime()));
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400">Loading bar orders...</p>
            </div>
          </div>
        </div>
      </BackgroundWrapper>
  ...
  return (
    <BackgroundWrapper variant="kitchen">
      <GlobalHeader 
        title="Bar Dashboard" 
        subtitle="Drink Orders & Beverages"
        icon={Wine}
        variant="kitchen"
        showBackButton
        onBack={onBack}
      ...
      <div className="container mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card className="text-center bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl border border-white/50 dark:border-gray-700/50">
            <Wine className="w-8 h-8 mx-auto mb-2 text-purple-500" />
        ...
        {/* Orders */}
        <div>
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Bar Tickets</h2>
          ...
          {orders.length === 0 ? (
            <Card className="text-center py-12 bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl border border-white/50 dark:border-gray-700/50">
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No drink orders</h3>
              <p className="text-gray-600 dark:text-gray-400">All caught up! New drink orders will appear here.</p>
            </Card>
          ) : (
